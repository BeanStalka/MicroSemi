...
void main (int argc, char** argv)
{
...
#ifdef LOAD_FWR_STATIC
   if (hbi_devices_info[deviceId].pFirmware == NULL)
   {
   ...
   status=vproc_load_image(handle, hbi_devices_info[deviceId].pFirmware);
...
#if LOAD_CFGREC_STATIC
   if (hbi_devices_info[deviceId].pConfig == NULL)
   {
   ...
   status=vproc_load_image(handle,hbi_devices_info[deviceId].pConfig);
   if(status == HBI_STATUS_SUCCESS)
   {
   ...
   else 
   {
      if(cfgrecLoaded)
      {
         /* Apply a soft reset. required if only config record is loaded
            by host
            */
  ...
  if(cfgrecLoaded && bSaveToFlash && (fwrLoaded==0))  
  {
         if ((argv[argc-1] == "-s")) {
             ...
             imageNum = (int)strtol(argv[argc-1],NULL,16);
