...
/*Example Initialization to support two ZL380xx devices on SPI bus 0, Chip select addr 0 and 1*/
/*To initialize it for I2c simply update the bus_num , and dev_addr accordingly*/
...
int hbi_test(int argc, char** argv) {
   ...
   devcfg.dev_addr = hbi_devices_info[deviceId].dev_addr;
   devcfg.bus_num = hbi_devices_info[deviceId].bus_num;
