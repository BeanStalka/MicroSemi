...
struct ssl_drv{
    struct ssl_lock     lock[NUM_MAX_LOCKS]; /* an array of locks hold by driver */
    struct ssl_dev      dev[VPROC_MAX_NUM_DEVS]; /* number of devices hold by driver */
...
ssl_status_t SSL_init(ssl_drv_cfg_t *pCfg)
{
    ...
    please note, we are not maintaining any reference count here.
    if there are multiple calls to init. all will simply be returned okay 
    similarly multiple calls to term will also return okay if driver is 
    terminated already */
                              ...
                              const char *pName, void *pOption)
{
   ...
   if(pLock == NULL)
    ...
    if(pName != NULL)
    {
        for(j=0;(pName[j]!='\0') || (j<sizeof(ssl_drv_priv.lock[i].name));j++)
        {
            ssl_drv_priv.lock[i].name[j]=pName[j];
        }
    }
...
//         OS_eEnterCriticalSection(&(((struct ssl_lock*)lock_id)->lock));
      break;
      default:
         VPROC_DBG_PRINT(VPROC_DBG_LVL_ERR,
...
//    OS_eExitCriticalSection(&(((struct ssl_lock*)lock_id)->lock));
