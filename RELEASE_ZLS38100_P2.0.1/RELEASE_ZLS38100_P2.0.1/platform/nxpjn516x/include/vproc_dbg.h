...
#include "dbg.h"  /*NXP debug from their SDK*/
/* Bitmask for selecting debug level information */
typedef enum
{
    VPROC_DBG_LVL_NONE=0x0,
    VPROC_DBG_LVL_FUNC=0x1,
    VPROC_DBG_LVL_INFO=0x1,
    VPROC_DBG_LVL_WARN=0x1,
    VPROC_DBG_LVL_ERR=0x1,
...
extern VPROC_DBG_LVL vproc_dbg_lvl;
#define VPROC_DBG_SET_LVL(dbg_lvl) (vproc_dbg_lvl = dbg_lvl)
/*#define VPROC_DBG_PRINT(level,msg,args...)  if(level & vproc_dbg_lvl) {debug_printf(level, "[%s:%d]"msg,__FUNCTION__,__LINE__,##args);} */
#define VPROC_DBG_PRINT DBG_vPrintf
#else
